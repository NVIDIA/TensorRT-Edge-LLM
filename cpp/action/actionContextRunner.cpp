...
bool ActionContextRunner::copyLmHiddenFrom(rt::Tensor const& lmHidden, cudaStream_t stream, int32_t validSeqLen)
{
    ...
    {
        LOG_ERROR("ActionContextRunner: invalid validSeqLen=%d for padded copy (dst seq=%d)", validSeqLen, dstSeqLen);
        return false;
    }
    if (lmHidden.getShape()[0] != batch || lmHidden.getShape()[2] != hidden || lmHidden.getShape()[1] < validSeqLen)
    {
